 ...
 */
function Shader(context, vertexSource, fragmentSource, macros) {
    ...
    this.updateShader(vertexSource, fragmentSource, macros);
}
...
Shader.prototype.updateShader = function(vertexSource, fragmentSource, macros)
{
    ...
    if (vertexSource.constructor === String) {
        if (macros) {
            vertexSource = macros + "\n" + vertexSource;
        }
        this.vs_shader = this.compileSource(gl.VERTEX_SHADER, vertexSource, this.vs_shader);
    } else {
        this.vs_shader = vertexSource;
    }

    if (fragmentSource.constructor === String) {
        if (macros) {
            fragmentSource = macros + "\n" + fragmentSource;
        }
        this.fs_shader = this.compileSource(gl.FRAGMENT_SHADER, fragmentSource, this.fs_shader);
    } else {
        this.fs_shader = fragmentSource;
    }

    gl.attachShader(this.program, this.vs_shader, gl);
    gl.attachShader(this.program, this.fs_shader, gl);
...
Shader.prototype.extractShaderInfo = function()
{
	...
	var func = this.getUniformFunc(data);
	var is_matrix = false;
 ...
 * @return {Function}
 */
Shader.prototype.getUniformFunc = function(data) {
    var gl = this.context.gl;
    
    var func = null;
    switch (data.type)
    {
	case gl.FLOAT: func = data.size == 1 ? gl.uniform1f : gl.uniform1fv; break;
	...
	case gl.INT: func = data.size == 1 ? gl.uniform1i : gl.uniform1iv; break;
	...
	case gl.SAMPLER_2D:
	case gl.SAMPLER_CUBE: func = gl.uniform1i; break;
...
Shader.prototype._setUniform = function(name, value)
{
    ...
    if (value.constructor === Array) {
        throw "value should be typed array";
    }
 ...
 */
Shader.prototype.compileSource = function(type, source, shader) {
    var gl = this.context.gl;

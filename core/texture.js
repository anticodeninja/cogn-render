...
function Texture(context, width, height, options) {
    options = options || {};
    ...
    this.format = options.format || this.context.DEFAULT_TEXTURE_FORMAT; //gl.RGBA (if gl.DEPTH_COMPONENT remember format: gl.UNSIGNED_SHORT)
    this.type = options.type || this.context.DEFAULT_TEXTURE_TYPE; //gl.UNSIGNED_BYTE, gl.UNSIGNED_SHORT, gl.FLOAT or gl.HALF_FLOAT_OES (or gl.HIGH_PRECISION_FORMAT which could be half or float)
    this.magFilter = options.magFilter || options.filter || this.context.DEFAULT_TEXTURE_MAG_FILTER;
    this.minFilter = options.minFilter || options.filter || this.context.DEFAULT_TEXTURE_MIN_FILTER;
    this.wrapS = options.wrap || options.wrapS || this.context.DEFAULT_TEXTURE_WRAP_S; 
    this.wrapT = options.wrap || options.wrapT || this.context.DEFAULT_TEXTURE_WRAP_T;
    ...
    if( (!Texture.isPowerOfTwo(this.width) || !Texture.isPowerOfTwo(this.height)) && //non power of two
	...
	gl.activeTexture(gl.TEXTURE0 + this.maxTextureUnits - 1);
	...
	if(this.texture_type == gl.TEXTURE_2D)
	{
	    ...
	    if (Texture.isPowerOfTwo(width) &&
                Texture.isPowerOfTwo(height) &&
                options.minFilter &&
...
{
    return this.gl.extensions["WEBGL_depth_texture"] != null;
}
...
Texture.prototype.setParameter = function(param, value) {
    ...
    switch(param)
    {
	case core.gl.TEXTURE_MAG_FILTER: this.magFilter = value; break;
	case core.gl.TEXTURE_MIN_FILTER: this.minFilter = value; break;
	case core.gl.TEXTURE_WRAP_S: this.wrapS = value; break;
	case core.gl.TEXTURE_WRAP_T: this.wrapT = value; break;
    }
}
 ...
 * @return {Texture} the texture
 */
Texture.fromImage = function(gl, image, options) {
    options = options || {};
    ...
    if (gl.isPowerOfTwo(texture.width) &&
        gl.isPowerOfTwo(texture.height) &&
        options.minFilter &&
        options.minFilter != gl.NEAREST &&
        options.minFilter != gl.LINEAR) {
	texture.bind();
...
Texture.isPowerOfTwo = function(v) {
